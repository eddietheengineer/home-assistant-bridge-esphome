...
3. Explicitly zeroes all entry fields (erd, data_size, uses_heap, update_required, publish_cooldown, valid) — avoids UBSan issues with bool fields after `memset`
...
If heap allocation fails, the entry is marked `valid = false` and `update_required = false`, a warning is logged, and the function returns `false`. No data is stored.
...
4. Initialize entry: set `erd`, `valid = true`, `update_required = true`, `uses_heap = false`, `data_size = 0`, `publish_cooldown = 0`
5. **Storage selection:**
   - If `data_size ≤ ERD_CACHE_INLINE_DATA_SIZE`: `memcpy` into `inline_data`, set `data_size`
   - If `data_size > ERD_CACHE_INLINE_DATA_SIZE`: allocate heap via `new (std::nothrow)`, copy data, set `uses_heap = true` and `data_size`; on allocation failure, mark entry `valid = false`, `update_required = false`, log a warning, and return `false`
6. Return `true`
...
2. **Heap allocation failure invalidates the entry:** If `new` fails for a large ERD, the entry is marked `valid = false` and `update_required = false`, and the update returns `false`. The data is not stored at all.
