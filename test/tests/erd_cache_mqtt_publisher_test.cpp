...
TEST(erd_cache_mqtt_publisher, loop_publishes_updated_erd)
{
  ...
  uint8_t data = 0x42;
  erd_cache_update(&cache, 0x0008, &data, sizeof(data), true, false);
...
TEST(erd_cache_mqtt_publisher, loop_respects_max_publishes)
{
  ...
  for (uint16_t i = 0; i < 20; i++) {
    ...
    erd_cache_update(&cache, (tiny_erd_t)(0x1000 + i), &data, sizeof(data), true, false);
  }
...
TEST(erd_cache_mqtt_publisher, loop_advances_publish_index)
{
  ...
  for (uint16_t i = 0; i < 10; i++) {
    uint8_t data = (uint8_t)i;
    erd_cache_update(&cache, (tiny_erd_t)(0x2000 + i), &data, sizeof(data), true, false);
  }
...
TEST(erd_cache_mqtt_publisher, loop_respects_time_budget)
{
  ...
  for (uint16_t i = 0; i < 50; i++) {
    uint8_t data = (uint8_t)i;
    erd_cache_update(&cache, (tiny_erd_t)(0x3000 + i), &data, sizeof(data), true, false);
  }
...
TEST(erd_cache_mqtt_publisher, loop_publishes_multiple_erds)
{
  ...
  uint8_t data_b = 0x02;
  uint8_t data_c = 0x03;
  erd_cache_update(&cache, 0x4001, &data_a, sizeof(data_a), true, false);
  erd_cache_update(&cache, 0x4002, &data_b, sizeof(data_b), true, false);
  erd_cache_update(&cache, 0x4003, &data_c, sizeof(data_c), true, false);
