...
| `erd_cache_mqtt_publisher_stop(self)` | Stop the background publishing task (ESP-IDF only; no-op otherwise). Dual-core safe shutdown via `done_semaphore` handshake. |
...
**Dual-core safety:**
- `state_mutex` protects shared state (`mqtt_connected`, `cache`, `mqtt_client`, `device_id`, `get_time_ms`) from concurrent access by the main loop.
- `done_semaphore` provides a dual-core safe shutdown handshake: the task gives the semaphore before calling `vTaskDelete`, so `stop()` can wait for true termination.
...
6. `stop()` sets `task_running = false`, signals work, waits on `done_semaphore` (1 s timeout)
...
- **Dual-core safety**: The ESP32-C3 is dual-core; the background task runs on one core while the main loop runs on the other. `state_mutex` protects shared state, and `done_semaphore` ensures safe shutdown.
