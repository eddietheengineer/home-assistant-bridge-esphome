...
AUTODISCOVERY_WAITING_5S
  └─ after 5 s delay →
    AUTODISCOVERY_GEA3_BROADCAST_PENDING → AUTODISCOVERY_GEA3_BROADCAST_WAITING
      ...
      └─ timeout → retry (up to 3) → GEA2 fallback or re-try GEA3
    ...
    AUTODISCOVERY_GEA2_BROADCAST_PENDING → AUTODISCOVERY_GEA2_BROADCAST_WAITING
      ...
      └─ timeout → retry (up to 3) → GEA3 fallback or re-try GEA2

AUTODISCOVERY_COMPLETE (terminal state)
```
...
- `AUTODISCOVERY_MAX_RETRIES = 3` — maximum retry attempts
