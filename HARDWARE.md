...
- **No dual-core concurrency on C3/C6.** The ESP32-C3 and ESP32-C6 each have a single CPU core. FreeRTOS tasks share the core through preemptive context switches — they never run in parallel. On dual-core ESP32-S3 the MQTT publisher task is pinned to Core 0 (off ESPHome's loop core, priority 1) so it cannot starve the loopTask watchdog. The ERD cache is protected by its own mutex (`erd_cache_t.lock`), so the main loop and publisher are safe whether they share a core or run in parallel; the lock is held only for the short entry read/copy or write, never across the slow MQTT publish. Synchronization primitives (`state_mutex`, `done_semaphore`, `lock`) protect against torn reads/writes during context switches on all platforms.
