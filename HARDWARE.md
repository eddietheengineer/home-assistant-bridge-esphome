...
- **No PSRAM.** All data structures, buffers, and task stacks must fit in the 320 KB of on-chip SRAM shared with the OS and ESPHome framework. This is why the MQTT publisher task uses a statically allocated 2 KB stack (`xTaskCreateStatic`), why ERD data is hex-encoded into fixed-size buffers, and why HA discovery entity definitions are embedded as compressed byte arrays rather than fetched at runtime.
...
- **Flash budget:** ~1.5 MB of 1.8 MB app partition. The embedded HA discovery JSONL data (~421 KB compressed) is the single largest consumer. Any new embedded data must be balanced against this ceiling.
